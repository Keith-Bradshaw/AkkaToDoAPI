...
object TodoRepository{
  final case class TodoNotFound(id: String) extends Exception(s"Todo with id $id cannot be found.")
}
...
class InMemoryTodoRepository(initialTodos: Seq[Todo] = Seq.empty)(implicit ec: ExecutionContext) extends TodoRepository{
  ...
  //If the title exists, update the old tdo with it, else keep it for next step
